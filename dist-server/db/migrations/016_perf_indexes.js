...
export async function up(connection) {
    ...
    if (!(await indexExists(connection, 'ticket_mensagens', 'idx_mensagens_ticket_interno_id'))) {
        await connection.query('ALTER TABLE ticket_mensagens ADD INDEX idx_mensagens_ticket_interno_id (ticket_id, interno, id)');
    }
    ...
    if (!(await indexExists(connection, 'tickets', 'idx_tickets_empresa_status'))) {
        await connection.query('ALTER TABLE tickets ADD INDEX idx_tickets_empresa_status (empresa_id, status)');
    }
    ...
    if (!(await indexExists(connection, 'tickets', 'idx_tickets_prazo_sla'))) {
        await connection.query('ALTER TABLE tickets ADD INDEX idx_tickets_prazo_sla (empresa_id, prazo_sla)');
    }
}
