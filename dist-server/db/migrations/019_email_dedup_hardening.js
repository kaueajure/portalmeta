...
async function duplicateMessageIdCount(connection, table) {
    const [rows] = await connection.query(`
      SELECT COUNT(*) AS count
      FROM (
        SELECT message_id
...
export async function up(connection) {
    ...
    if (duplicateTicketMessages === 0) {
        if (!(await indexExists(connection, 'ticket_mensagens', 'uniq_ticket_mensagens_message_id'))) {
            await connection.query('ALTER TABLE ticket_mensagens ADD UNIQUE INDEX uniq_ticket_mensagens_message_id (message_id)');
        }
    }
    else {
        console.warn(`[MIGRATE] ticket_mensagens possui ${duplicateTicketMessages} Message-ID(s) duplicado(s); indice unico nao foi criado.`);
    }
    ...
    if (duplicateTickets === 0) {
        if (!(await indexExists(connection, 'tickets', 'uniq_tickets_message_id'))) {
            await connection.query('ALTER TABLE tickets ADD UNIQUE INDEX uniq_tickets_message_id (message_id)');
        }
    }
    else {
        console.warn(`[MIGRATE] tickets possui ${duplicateTickets} Message-ID(s) duplicado(s); indice unico nao foi criado.`);
    }
}
