...
function splitNameAndSigla(rawName) {
...
export async function up(connection) {
    ...
    const [rows] = await connection.query('SELECT id, nome, sigla FROM empresa_ticket_categorias');
    for (const row of rows) {
        ...
        if (!parsed.sigla)
            continue;
        await connection.query('UPDATE empresa_ticket_categorias SET nome = ?, sigla = ? WHERE id = ?', [parsed.nome, parsed.sigla, row.id]);
    }
}
