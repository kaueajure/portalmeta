...
const OBLIGATIONS = ['MSC', 'SIOPE', 'SIOPS', 'RREO', 'RGF', 'DCA'];
...
function municipalityDisplayName(value) {
...
function childFingerprint(values) {
...
function sqlStatements(sql) {
    ...
    for (let index = 0; index < sql.length; index += 1) {
        ...
        if (quoted) {
            ...
            if (insertStart >= 0)
                statements.push(statement.slice(insertStart));
            start = index + 1;
        }
    }
    return statements;
    ...
    for (const statement of sqlStatements(sql)) {
        ...
        for (const tuple of parseTuples(values)) {
            if (tuple.length !== columns.length)
                throw new Error(`Quantidade de colunas inválida em ${table}.`);
...
function activeServices(raw) {
    ...
    try {
        ...
        if (!configured || typeof configured !== 'object')
            return allActive;
        return Object.fromEntries(OBLIGATIONS.map((code) => [code, configured[code] !== false]));
    }
    catch {
        return allActive;
    }
...
async function applyImport({ connection, municipalities, eligibleTasks, sourceHistories, sourceComments, userByIdentity, targetDispositionBySourceId, }) {
    const applied = { municipalitiesInserted: 0, tasksInserted: 0, tasksUpdated: 0, historiesInserted: 0, commentsInserted: 0 };
    await connection.beginTransaction();
    try {
        const [existingMunicipalities] = await connection.query('SELECT id, name, state FROM obligation_municipalities FOR UPDATE');
        const existingKeys = new Set(existingMunicipalities.map((row) => `${normalizedName(row.name)}|${String(row.state).toUpperCase()}`));
        ...
        const [currentMunicipalities] = await connection.query('SELECT id, name, state FROM obligation_municipalities WHERE active = 1');
        const municipalityIdByKey = new Map(currentMunicipalities.map((row) => [
        ...
        const taskInsertValues = eligibleTasks
            ...
            const municipalityId = municipalityIdByKey.get(`${normalizedName(task.municipalityName)}|SP`);
            if (!municipalityId)
                throw new Error(`Município não localizado durante a aplicação: ${task.municipalityName}`);
        ...
        if (taskInsertValues.length > 0) {
            const [result] = await connection.query(`INSERT INTO obligation_tasks
        ...
        const [currentTasks] = await connection.query(`
    ...
    `);
        const targetTaskIdByKey = new Map(currentTasks.map((row) => [taskKey({
        ...
        const targetTaskIdBySourceId = new Map();
        for (const task of eligibleTasks) {
            const targetId = targetTaskIdByKey.get(taskKey(task));
            if (!targetId)
                throw new Error(`Tarefa não localizada após inserção: ${taskKey(task)}`);
    ...
    `);
        const historyFingerprints = new Set(existingHistoryRows.map((row) => childFingerprint([
        ...
        if (historyValues.length > 0) {
            const [result] = await connection.query(`INSERT INTO obligation_task_history
        ...
        const [existingCommentRows] = await connection.query(`
      SELECT task_id AS taskId, author_name AS authorName, text,
        DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') AS createdAt
      FROM obligation_task_comments
    `);
        const commentFingerprints = new Set(existingCommentRows.map((row) => childFingerprint([
        ...
        if (commentValues.length > 0) {
            const [result] = await connection.query(`INSERT INTO obligation_task_comments
...
async function main() {
    ...
    const municipalities = (tables.municipalities || []).map((row) => ({
        id: Number(row.id),
    ...
    const tasks = (tables.tasks || []).map((row) => ({
        id: Number(row.id),
    ...
    for (const municipality of municipalities) {
        ...
        if (municipalityKeys.has(key))
            validationErrors.push(`Município duplicado: ${municipality.name}/${municipality.state}`);
        municipalityKeys.add(key);
        if (municipality.state !== 'SP')
            validationErrors.push(`UF diferente de SP: ${municipality.name}/${municipality.state}`);
    }
    const sourceTaskKeys = new Set();
    for (const task of tasks) {
        if (!municipalityById.has(task.municipalityId))
            validationErrors.push(`Tarefa ${task.id} sem município.`);
        if (!OBLIGATIONS.includes(task.obligationCode))
            validationErrors.push(`Tarefa ${task.id} com obrigação inválida: ${task.obligationCode}`);
        if (!VALID_STATUSES.has(task.status))
            validationErrors.push(`Tarefa ${task.id} com status inválido: ${task.status}`);
        if (task.siopsMembros && !VALID_SIOPS.has(task.siopsMembros))
            validationErrors.push(`Tarefa ${task.id} com SIOPS inválido: ${task.siopsMembros}`);
        if (task.siopeFolha && !VALID_SIOPE.has(task.siopeFolha))
            validationErrors.push(`Tarefa ${task.id} com SIOPE inválido: ${task.siopeFolha}`);
        const key = taskKey(task);
        if (sourceTaskKeys.has(key))
            validationErrors.push(`Tarefa duplicada: ${key}`);
    ...
    try {
        const [tableRows] = await pool.query(`SELECT TABLE_NAME AS tableName FROM information_schema.TABLES
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME IN (?)`, [REQUIRED_TARGET_TABLES]);
        const targetTables = new Set(tableRows.map((row) => row.tableName));
        ...
        INNER JOIN obligation_municipalities m ON m.id = t.municipality_id
      `);
            const [userRows] = await pool.query("SELECT id, nome, email FROM usuarios WHERE ativo = 1 AND COALESCE(perfil, '') <> 'cliente'");
            const targetMunicipalities = targetMunicipalityRows;
            ...
            const targetTasks = targetTaskRows;
            ...
            for (const municipality of municipalities) {
                if (targetMunicipalityByKey.has(`${normalizedName(municipality.name)}|${municipality.state}`))
                    municipalityPlan.reuse += 1;
                else {
            ...
            const targetDispositionBySourceId = new Map();
            for (const task of eligibleTasks) {
                ...
                if (safe) {
                    ...
                    if (Number(target.historyCount) > 0)
                        conflictProtections.withHistory += 1;
                    if (Number(target.commentCount) > 0)
                        conflictProtections.withComments += 1;
                    if (Number(target.attachmentCount) > 0)
                        conflictProtections.withAttachments += 1;
                }
            }
            ...
            for (const user of userRows) {
                ...
                if (user.nome)
                    userByIdentity.set(identityKey(String(user.nome)), mapped);
                if (user.email)
                    userByIdentity.set(String(user.email).trim().toLowerCase(), mapped);
            }
            ...
            for (const row of sourceHistories) {
                ...
                if (userByIdentity.has(identityKey(author)) || userByIdentity.has(author.trim().toLowerCase()))
                    historyMatched += 1;
                else
                    historySemRegistro += 1;
            }
            for (const row of sourceComments) {
            ...
            if (apply) {
                if (validationErrors.length > 0)
                    throw new Error('A aplicação foi bloqueada por erros de validação do dump.');
                if (taskPlan.conflict > 0)
                    throw new Error(`A aplicação foi bloqueada por ${taskPlan.conflict} conflito(s).`);
