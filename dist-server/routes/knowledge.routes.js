...
router.get('/categories', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT DISTINCT categoria FROM knowledge_articles ORDER BY categoria ASC');
...
router.post('/', requireAnyPermission(['base_conhecimento.criar', 'base_conhecimento.gerenciar']), async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO knowledge_articles (titulo, conteudo, categoria, publico, ativo, created_by) VALUES (?, ?, ?, ?, ?, ?)', [titulo, conteudo, categoria || null, publico ? 1 : 0, ativo !== undefined ? ativo : 1, req.user.id]);
...
router.patch('/:id', requireAnyPermission(['base_conhecimento.editar', 'base_conhecimento.gerenciar']), async (req, res) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id FROM knowledge_articles WHERE id = ?', [id]);
