...
async function portalOwnsTicket(ticketId, context) {
    if (!context)
        return null;
...
async function savePortalFiles(files, ticketId, messageId, userId) {
...
router.get('/options', async (_req, res) => {
    try {
        const [categories] = await pool.query('SELECT id, nome, sigla, valor, ativo, ordem FROM ticket_categories WHERE ativo = 1 ORDER BY ordem, id');
        const [services] = await pool.query('SELECT id, nome, valor, ativo, ordem, formulario_json FROM ticket_services WHERE ativo = 1 ORDER BY ordem, id');
...
router.get('/tickets/:id/messages', async (req, res) => {
    ...
    try {
        ...
        if (!await portalOwnsTicket(Number(req.params.id), context))
            return sendError(res, 'Chamado não encontrado', 404);
    ...
    `;
        const messageParams = [context.usuario_id, context.usuario_id, context.mode, req.params.id];
        ...
        const attachmentsByMessage = await attachmentsService.getByMessages(rows.map((row) => Number(row.id)), false, Number(req.params.id));
        rows.forEach((row) => {
...
router.post('/tickets', ticketUpload.array('files', 5), async (req, res) => {
...
router.post('/tickets/:id/messages', ticketUpload.array('files', 5), async (req, res) => {
    const files = (req.files || []);
    const context = getPortalContext(req);
    if (!context)
        return sendError(res, 'Não autorizado', 401);
    const { mensagem } = req.body;
    if (!String(mensagem || '').trim() && files.length === 0)
