...
const normalizeCategorySigla = (value) => typeof value === 'string' ? value.trim().toUpperCase().slice(0, CATEGORY_SIGLA_MAX_LENGTH) : '';
const isSiglaTooLong = (value) => typeof value === 'string' && value.trim().length > CATEGORY_SIGLA_MAX_LENGTH;
const slugifyOptionValue = (value) => String(value || '')
...
router.get('/categories', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT * FROM ticket_categories ORDER BY ordem ASC, id ASC');
...
router.post('/categories', async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO ticket_categories (nome, sigla, valor, ativo, ordem) VALUES (?, ?, ?, ?, ?)', [normalizedNome, sigla, normalizedValor, ativo !== undefined ? ativo : 1, ordem || 0]);
...
router.get('/services', async (req, res) => {
    try {
        ...
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
        const [rows] = await pool.query('SELECT * FROM ticket_services ORDER BY ordem ASC, id ASC');
        sendSuccess(res, rows);
    }
    catch (error) {
...
router.patch('/services/:servId', async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
...
router.delete('/services/:servId', async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
...
router.get('/statuses/usage', async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
...
router.put('/statuses', async (req, res) => {
    ...
    try {
        ...
        const [oldRows] = await connection.query('SELECT valor FROM ticket_statuses');
        ...
        for (const status of sanitized) {
            await connection.query(`INSERT INTO ticket_statuses
	           (nome, valor, ativo, kanban_visivel, cor, especial, ordem)
	           VALUES (?, ?, ?, ?, ?, ?, ?)`, [status.nome, status.valor, status.ativo, status.kanban_visivel, status.cor, status.especial, status.ordem]);
        }
        ...
        if (sanitized.length > 0) {
            ...
            for (const oldStatus of oldStatusValues) {
                ...
                if (configuredStatusValues.includes(oldStatus))
                    continue;
                const [explicitRows] = await connection.query('SELECT id FROM tickets WHERE status = ?', [oldStatus]);
                affectedTicketIds.push(...explicitRows.map((r) => Number(r.id)));
                await connection.query(`UPDATE tickets
             SET status = ?, updated_at = NOW()
             WHERE status = ?`, [targetStatus, oldStatus]);
            }
            ...
            const [affectedRows] = await connection.query(`SELECT id FROM tickets
	         WHERE status NOT IN (${placeholders})`, configuredStatusValues);
            affectedTicketIds.push(...affectedRows.map((r) => Number(r.id)));
            await connection.query(`UPDATE tickets
	         SET status = ?, updated_at = NOW()
	         WHERE status NOT IN (${placeholders})`, [fallbackStatus, ...configuredStatusValues]);
        }
...
router.post('/sla-policies', async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO sla_policies (nome, prioridade, categoria, servico, tempo_primeira_resposta_minutos, tempo_resolucao_minutos, ativo, ordem) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [nome, prioridade || null, categoria || null, servico || null, tempo_primeira_resposta_minutos || null, tempo_resolucao_minutos || 24 * 60, ativo !== undefined ? ativo : 1, ordem || 0]);
        sendSuccess(res, { id: result.insertId });
    }
    catch (error) {
...
router.patch('/sla-policies/:policyId', async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
        ...
        if (!hasConfigPerm)
            return sendError(res, 'Acesso negado', 403);
        const { nome, prioridade, categoria, servico, tempo_primeira_resposta_minutos, tempo_resolucao_minutos, ativo, ordem } = req.body;
        await pool.query('UPDATE sla_policies SET nome = ?, prioridade = ?, categoria = ?, servico = ?, tempo_primeira_resposta_minutos = ?, tempo_resolucao_minutos = ?, ativo = ?, ordem = ? WHERE id = ?', [nome, prioridade || null, categoria || null, servico || null, tempo_primeira_resposta_minutos || null, tempo_resolucao_minutos || 24 * 60, ativo !== undefined ? ativo : 1, ordem || 0, policyId]);
...
router.delete('/sla-policies/:policyId', async (req, res) => {
    try {
        const currentUser = req.user;
        if (!currentUser)
            return sendError(res, 'Não autenticado', 401);
