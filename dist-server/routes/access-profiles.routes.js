...
router.get('/:id/matrix', requirePermission('usuarios.ver_permissoes'), async (req, res) => {
    try {
        ...
        if (access.error)
            return sendError(res, access.error, access.status);
...
router.patch('/:id', requirePermission('usuarios.gerenciar_permissoes'), async (req, res) => {
    try {
        const id = parsePositiveInt(req.params.id);
        if (!id)
            return sendError(res, 'ID invalido', 400);
        ...
        if (Array.isArray(req.body.permissions)) {
            const [users] = await pool.query('SELECT id FROM usuarios WHERE access_profile_id = ?', [id]);
...
router.delete('/:id', requirePermission('usuarios.gerenciar_permissoes'), async (req, res) => {
    try {
        const id = parsePositiveInt(req.params.id);
        if (!id)
            return sendError(res, 'ID invalido', 400);
        ...
        if (access.error)
            return sendError(res, access.error, access.status);
        const [users] = await pool.query('SELECT COUNT(*) as total FROM usuarios WHERE access_profile_id = ?', [id]);
