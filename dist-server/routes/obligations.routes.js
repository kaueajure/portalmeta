...
function parseResponsibleConfig(value) {
    if (!value)
        return {};
...
function sanitizeResponsibleConfig(value, validUserNames) {
    ...
    const sanitized = { _activeServices: { ...config._activeServices } };
...
function splitResponsibles(config, obligationCode) {
...
function isOverdue(status, obligationCode, competence, year) {
...
function mapMunicipality(row, validUserNames) {
    return {
        id: Number(row.id),
        name: row.name,
        state: row.state,
...
function normalizeMunicipalityPayload(body) {
    ...
    const source = body.responsibleConfig && typeof body.responsibleConfig === 'object'
        ...
        : {};
    const responsibleConfig = { _activeServices: {} };
    for (const code of Object.keys(OBLIGATIONS)) {
...
router.get('/dashboard', requirePermission('obrigacoes.dashboard.visualizar'), async (req, res) => {
    ...
    try {
        ...
        const [municipalityRows] = await connection.query(`SELECT id, name, state, responsible_config
        ...
        for (const municipality of municipalityRows) {
            municipality.responsible_config = sanitizeResponsibleConfig(municipality.responsible_config, validUserNames);
        }
        const values = [];
        for (const municipality of municipalityRows) {
            for (const [code, competences] of Object.entries(OBLIGATIONS)) {
                if (!isServiceActive(municipality.responsible_config, code))
                    continue;
        ...
        const [taskRows] = await connection.query(`SELECT id, municipality_id, obligation_code, competence, year, status
         FROM obligation_tasks WHERE year = ?`, [year]);
        const [completionRows] = await connection.query(`SELECT h.task_id, h.actor_name
        ...
        const completedByTask = new Map(completionRows
            .filter((row) => String(row.actor_name || '').trim())
            .map((row) => [Number(row.task_id), String(row.actor_name).trim()]));
        ...
        for (const task of activeTasks) {
            ...
            for (const name of responsibles) {
                const stats = responsibleMap.get(name) || { completed: 0, pending: 0, municipalities: new Set() };
...
router.get('/municipalities', requirePermission('obrigacoes.municipios.visualizar'), async (_req, res) => {
    try {
        const [municipalityResult, userResult] = await Promise.all([
            pool.query(`SELECT id, name, state, responsible_config, phone, email, observations
           FROM obligation_municipalities WHERE active = 1 ORDER BY name`),
            pool.query(`SELECT id, nome, email, cargo
           FROM usuarios
           WHERE ativo = 1 AND COALESCE(perfil, '') <> 'cliente'
           ORDER BY nome`),
        ]);
        ...
        const validUserNames = userRows.map((row) => String(row.nome));
        return sendSuccess(res, {
            municipalities: municipalityResult[0].map((row) => mapMunicipality(row, validUserNames)),
...
router.post('/municipalities', requirePermission('obrigacoes.municipios.criar'), async (req, res) => {
    ...
    try {
        const [result] = await pool.query(`INSERT INTO obligation_municipalities
          (name, state, responsible_config, phone, email, observations)
         VALUES (?, ?, ?, ?, ?, ?)`, [payload.name, payload.state, JSON.stringify(payload.responsibleConfig), payload.phone, payload.email, payload.observations]);
        const [rows] = await pool.query(`SELECT id, name, state, responsible_config, phone, email, observations
         FROM obligation_municipalities WHERE id = ?`, [result.insertId]);
...
router.put('/municipalities/:id', requirePermission('obrigacoes.municipios.editar'), async (req, res) => {
    ...
    const payload = normalizeMunicipalityPayload(req.body);
    if (!municipalityId || !payload)
        return sendError(res, 'Dados do município inválidos.', 400);
    try {
        const [result] = await pool.query(`UPDATE obligation_municipalities
         SET name = ?, state = ?, responsible_config = ?, phone = ?, email = ?, observations = ?
         WHERE id = ? AND active = 1`, [payload.name, payload.state, JSON.stringify(payload.responsibleConfig), payload.phone, payload.email, payload.observations, municipalityId]);
        if (result.affectedRows === 0)
            return sendError(res, 'Município não encontrado.', 404);
        const [rows] = await pool.query(`SELECT id, name, state, responsible_config, phone, email, observations
         FROM obligation_municipalities WHERE id = ?`, [municipalityId]);
...
router.delete('/municipalities/:id', requirePermission('obrigacoes.municipios.excluir'), async (req, res) => {
    const municipalityId = positiveInt(req.params.id);
    if (!municipalityId)
        return sendError(res, 'Município inválido.', 400);
    try {
        const [result] = await pool.query('DELETE FROM obligation_municipalities WHERE id = ?', [municipalityId]);
        if (result.affectedRows === 0)
            return sendError(res, 'Município não encontrado.', 404);
...
router.get('/workspace', requirePermission('obrigacoes.planilha.visualizar'), async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        const [municipalityRows] = await connection.query(`SELECT id, name, state, responsible_config, phone, email, observations
         FROM obligation_municipalities WHERE active = 1 ORDER BY name`);
        const [userRows] = await connection.query(`SELECT nome FROM usuarios
         WHERE ativo = 1 AND COALESCE(perfil, '') <> 'cliente'`);
        const validUserNames = userRows.map((row) => String(row.nome));
        ...
        const activeMunicipalities = municipalityRows.filter((row) => isServiceActive(row.responsible_config, scope.obligationCode));
        ...
        const [taskRows] = await connection.query(`SELECT id, municipality_id, obligation_code, competence, year, status,
         ...
         FROM obligation_tasks WHERE year = ? AND obligation_code = ?`, [scope.year, scope.obligationCode]);
...
router.put('/tasks/:id', requirePermission('obrigacoes.planilha.editar'), async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query(`SELECT t.*, m.responsible_config FROM obligation_tasks t
        ...
        if (current.status === 'Falta XML' && status === 'Não iniciado') {
            ...
            for (const item of related) {
                ...
                if (!isServiceActive(current.responsible_config, code))
                    continue;
                await connection.query(`INSERT IGNORE INTO obligation_tasks
              (municipality_id, obligation_code, competence, year, status, siops_membros, siope_folha)
        ...
        const [updatedRows] = await connection.query('SELECT * FROM obligation_tasks WHERE id = ?', [taskId]);
        await connection.commit();
...
router.put('/history/:id', requirePermission('obrigacoes.planilha.editar_historico'), async (req, res) => {
    ...
    if (!historyId)
        return sendError(res, 'Histórico inválido.', 400);
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        ...
        if (newValue)
            await connection.query(`UPDATE obligation_tasks SET ${taskColumn} = ? WHERE id = ?`, [newValue, history.task_id]);
