...
export const whatsappService = {
    ...
    async updateBotSettings(input) {
        ...
        const next = {
            ...
            startMessage: String(input.startMessage !== undefined ? input.startMessage : current.startMessage).trim().slice(0, 2000),
    ...
    async startAttendance(phone, optionId, optionTitle, contactName) {
        ...
        try {
            ...
            await connection.query(`INSERT INTO whatsapp_sessions (contact_phone, contact_name, status)
         VALUES (?, ?, 'idle')
         ON DUPLICATE KEY UPDATE contact_name = COALESCE(VALUES(contact_name), contact_name)`, [normalized, contactName || null]);
            const [sessionRows] = await connection.query(`SELECT s.status, s.attendance_cycle_id, c.status AS cycle_status
         FROM whatsapp_sessions s
         LEFT JOIN whatsapp_attendance_cycles c ON c.id = s.attendance_cycle_id
         WHERE s.contact_phone = ? FOR UPDATE`, [normalized]);
            ...
            if ((current?.status === 'active' || current?.cycle_status === 'pending') && cycleId) {
                await connection.query(`UPDATE whatsapp_attendance_cycles
           SET status = 'active', service_id = ?, service_title = ?, contact_name = COALESCE(?, contact_name)
           WHERE id = ? AND status IN ('pending', 'active')`, [String(optionId || '').trim().toUpperCase().slice(0, 256), optionTitle, contactName || null, cycleId]);
            }
            else {
                const [cycleResult] = await connection.query(`INSERT INTO whatsapp_attendance_cycles
            (contact_phone, contact_name, service_id, service_title, status, started_at)
           VALUES (?, ?, ?, ?, 'active', NOW())`, [normalized, contactName || null, String(optionId || '').trim().toUpperCase().slice(0, 256), optionTitle]);
    ...
    async ensurePendingAttendanceCycle(phone, contactName) {
        const normalized = normalizePhone(phone);
        if (!normalized)
            return null;
        const connection = await pool.getConnection();
        try {
         ...
         VALUES (?, ?, 'idle')
         ON DUPLICATE KEY UPDATE contact_name = COALESCE(VALUES(contact_name), contact_name)`, [normalized, contactName || null]);
            const [rows] = await connection.query(`SELECT s.status, s.attendance_cycle_id, c.status AS cycle_status
         FROM whatsapp_sessions s
         LEFT JOIN whatsapp_attendance_cycles c ON c.id = s.attendance_cycle_id
         WHERE s.contact_phone = ? FOR UPDATE`, [normalized]);
            ...
            if (session?.cycle_status === 'pending' && session.attendance_cycle_id) {
                await connection.query(`UPDATE whatsapp_attendance_cycles SET contact_name = COALESCE(?, contact_name) WHERE id = ?`, [contactName || null, session.attendance_cycle_id]);
                await connection.commit();
                return Number(session.attendance_cycle_id);
            }
            const [result] = await connection.query(`INSERT INTO whatsapp_attendance_cycles
          (contact_phone, contact_name, status, started_at)
         VALUES (?, ?, 'pending', NOW())`, [normalized, contactName || null]);
            const cycleId = Number(result.insertId);
            await connection.query(`UPDATE whatsapp_sessions
         ...
         WHERE contact_phone = ?`, [cycleId, normalized]);
            await connection.commit();
            emitWhatsAppChanged();
            return cycleId;
        }
    ...
    async closeAttendance(phone) {
        ...
        try {
            await connection.beginTransaction();
            const [rows] = await connection.query(`SELECT attendance_cycle_id FROM whatsapp_sessions WHERE contact_phone = ? FOR UPDATE`, [normalized]);
            const cycleId = Number(rows[0]?.attendance_cycle_id) || null;
            if (cycleId) {
                await connection.query(`UPDATE whatsapp_attendance_cycles
           SET status = 'closed', closed_at = COALESCE(closed_at, NOW())
           WHERE id = ? AND status = 'active'`, [cycleId]);
            }
            await connection.query(`
          UPDATE whatsapp_sessions
          SET status = 'idle',
        ...
        `, [normalized]);
          ...
          `, [normalized, session.attendance_cycle_id, actorId, actorName]);
                if (session.attendance_cycle_id) {
                    await connection.query(`UPDATE whatsapp_attendance_cycles
             SET assigned_user_id = ?, assigned_at = NOW()
             WHERE id = ? AND status = 'active'`, [actorId, session.attendance_cycle_id]);
                }
            }
            await connection.commit();
            emitWhatsAppChanged();
    ...
    async getAttendanceCycle(cycleId) {
        const id = Number(cycleId);
        if (!Number.isInteger(id) || id <= 0)
            return null;
        const [rows] = await pool.query(`SELECT id, contact_phone, contact_name, service_id, service_title, status,
              started_at, closed_at, assigned_user_id, assigned_at, registered_ticket_id
       FROM whatsapp_attendance_cycles WHERE id = ? LIMIT 1`, [id]);
        return rows[0] || null;
    },
    async transferAttendance(phone, targetUserId, actor) {
        const normalized = normalizePhone(phone);
        ...
        try {
            await connection.beginTransaction();
            const [sessions] = await connection.query(`SELECT status, attendance_cycle_id, assigned_user_id FROM whatsapp_sessions WHERE contact_phone = ? FOR UPDATE`, [normalized]);
            ...
            if (Number(session.assigned_user_id) !== Number(actor.id)) {
                throw Object.assign(new Error('Somente o responsável atual pode transferir este atendimento.'), { status: 403 });
            }
            ...
            const [users] = await connection.query(`SELECT id, nome FROM usuarios WHERE id = ? AND ativo = 1 AND perfil <> 'cliente' LIMIT 1`, [targetId]);
            if (!users.length)
                throw Object.assign(new Error('Atendente não encontrado ou inativo.'), { status: 404 });
            await connection.query(`UPDATE whatsapp_sessions SET assigned_user_id = ?, assigned_at = NOW() WHERE contact_phone = ?`, [targetId, normalized]);
            await connection.query(`INSERT INTO whatsapp_assignment_history
          (contact_phone, attendance_cycle_id, user_id, user_name, assigned_at)
         VALUES (?, ?, ?, ?, NOW())`, [normalized, session.attendance_cycle_id, targetId, users[0].nome]);
            if (session.attendance_cycle_id) {
                await connection.query(`UPDATE whatsapp_attendance_cycles
           SET assigned_user_id = ?, assigned_at = NOW()
           WHERE id = ? AND status = 'active'`, [targetId, session.attendance_cycle_id]);
            }
            await connection.commit();
            emitWhatsAppChanged();
            return this.getAssignmentDetails(normalized);
    ...
    async finishAttendance(phone, actorId) {
        const normalized = normalizePhone(phone);
        const session = await this.getSession(normalized);
        if (!session || session.status !== 'active') {
    ...
    async registerConversationAsTicket(phone, actor) {
        const normalized = normalizePhone(phone);
        if (!normalized)
        ...
        if (cycle.registered_ticket_id)
            return { id: Number(cycle.registered_ticket_id), existing: true };
        ...
        await pool.query(`UPDATE whatsapp_attendance_cycles SET registered_ticket_id = ?
       WHERE id = ? AND registered_ticket_id IS NULL`, [ticketId, cycle.id]);
        await pool.query(`UPDATE whatsapp_sessions SET registered_ticket_id = ?
       WHERE contact_phone = ? AND attendance_cycle_id = ?`, [ticketId, normalized, cycle.id]);
        emitWhatsAppChanged();
                ...
                for (const msg of messages) {
                    ...
                    if (persisted.inserted && phone) {
                        ...
                        if (attendanceCycleId && persisted.id) {
                            await pool.query(`UPDATE whatsapp_messages SET attendance_cycle_id = ? WHERE id = ?`, [attendanceCycleId, persisted.id]);
                            emitWhatsAppChanged();
                        }
                        const session = await this.getSession(phone);
    ...
    async listAttendanceCycleMessages(phone, limit = 300, attendanceCycleId) {
        const normalized = normalizePhone(phone);
        if (!normalized)
            return [];
        const session = await this.getSession(normalized);
        const cycle = await this.getAttendanceCycle(attendanceCycleId || session?.attendance_cycle_id);
        if (!cycle || cycle.contact_phone !== normalized)
            return [];
        const safeLimit = Math.min(Math.max(Number(limit) || 300, 1), 500);
        const [rows] = await pool.query(`SELECT id, wa_message_id, direction, from_phone, to_phone, contact_name,
              message_type, body, status,
              DATE_FORMAT(created_at, '%Y-%m-%dT%H:%i:%sZ') AS created_at
       FROM whatsapp_messages
