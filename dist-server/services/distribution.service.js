...
export async function distributeTicket(ticket) {
    try {
        ...
        // Find matching rule
        const [regras] = await pool.query('SELECT * FROM distribution_rules WHERE ativo = 1 ORDER BY id ASC');
        ...
        if (Array.isArray(config.agents) && config.agents.length > 0) {
            ...
            if (ids.length > 0) {
                ...
                const [rows] = await pool.query(`SELECT id FROM usuarios WHERE ativo = 1 AND id IN (${placeholders}) AND (perfil IN ('atendente', 'gestor', 'administrador', 'desenvolvedor') OR administrador = 1)`, ids);
                agents = rows;
            }
        }
        if (agents.length === 0) {
            const [rows] = await pool.query(`SELECT id FROM usuarios WHERE ativo = 1 AND (perfil IN ('atendente', 'gestor', 'administrador', 'desenvolvedor') OR administrador = 1)`);
            agents = rows;
        }
        if (agents.length === 0)
            return null;
        ...
        if (matchedRule.metodo === 'menor_carga') {
         ...
         `, [agentIds]);
            ...
            const [lastTicketRes] = await pool.query('SELECT responsavel_id FROM tickets WHERE responsavel_id IS NOT NULL ORDER BY id DESC LIMIT 1');
