...
export function normalizeServiceForm(value: unknown): ServiceFormField[] {
  let parsed = value;
  if (typeof value === 'string') {
    try { parsed = JSON.parse(value); } catch { throw new Error('Formulario do servico invalido.'); }
  }
  if (parsed === null || parsed === undefined || parsed === '') return [];
  if (!Array.isArray(parsed) || parsed.length > 20) throw new Error('Formulario do servico invalido.');
  ...
  return parsed.map((raw: any, index) => {
    ...
    usedKeys.add(chave);
    const opcoes: string[] | undefined = tipo === 'selecao'
      ? Array.from(new Set<string>((Array.isArray(raw?.opcoes) ? raw.opcoes : [])
        .map((item: unknown) => String(item).trim()).filter((item: string) => Boolean(item)))).slice(0, 30)
      : undefined;
    if (tipo === 'selecao' && !opcoes?.length) throw new Error(`Informe as opcoes de "${rotulo}".`);
    return { chave, rotulo, tipo: tipo as ServiceFormField['tipo'], obrigatorio: Boolean(raw?.obrigatorio), opcoes };
  });
}

export async function validateServiceFormAnswers(service: unknown, answersValue: unknown) {
  let answers: Record<string, unknown> = {};
  if (typeof answersValue === 'string' && answersValue.trim()) {
    try { answers = JSON.parse(answersValue); } catch { throw new Error('Respostas do formulario invalidas.'); }
  } else if (answersValue && typeof answersValue === 'object' && !Array.isArray(answersValue)) {
    answers = answersValue as Record<string, unknown>;
  }

  if (!service) return [];
  const [rows]: any = await pool.query(
    'SELECT formulario_json FROM ticket_services WHERE valor = ? AND ativo = 1 LIMIT 1',
    [String(service)],
  );
  ...
  return schema.map(field => {
    ...
    if (field.obrigatorio && !value) throw new Error(`O campo "${field.rotulo}" e obrigatorio.`);
