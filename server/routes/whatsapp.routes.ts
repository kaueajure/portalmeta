...
router.post(
  '/conversations/:phone/transfer',
  requirePermission('integracoes.whatsapp.gerenciar', { allowDeveloper: true }),
  async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) return sendError(res, 'Usuário não autenticado', 401);
      const data = await whatsappService.transferAttendance(
        String(req.params.phone || ''),
        Number(req.body?.userId),
        { id: req.user.id, name: req.user.nome },
      );
...
router.post(
  '/conversations/:phone/ticket',
  requirePermission('tickets.criar'),
  async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) return sendError(res, 'Usuário não autenticado', 401);
      const data = await whatsappService.registerConversationAsTicket(
        String(req.params.phone || ''),
        { id: req.user.id, name: req.user.nome },
      );
      const io = req.app.get('io');
      if (io) io.to('instance').emit('ticketCreated', { id: data.id });
