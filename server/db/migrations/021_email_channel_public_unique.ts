...
async function duplicatePublicEmailCount(connection: PoolConnection): Promise<number> {
  const [rows]: any = await connection.query(
    `
      SELECT COUNT(*) AS count
      FROM (
...
export async function up(connection: PoolConnection) {
  ...
  if (duplicates > 0) {
    throw new Error(
      `empresa_email_canais possui ${duplicates} email_publico duplicado(s) normalizado(s). ` +
  ...
  if (!(await indexExists(connection, 'empresa_email_canais', 'uniq_email_canais_publico_normalized'))) {
    await connection.query(
      'ALTER TABLE empresa_email_canais ADD UNIQUE INDEX uniq_email_canais_publico_normalized (email_publico_normalized)'
    );
  }
}
