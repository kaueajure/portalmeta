...
async function constraintExists(connection: PoolConnection, table: string, constraint: string) {
  const [rows]: any = await connection.query(
...
function serviceConfigFromLegacy(value: unknown) {
  const legacy = parseConfig(value);
  return {
    activeServices: Object.fromEntries(
      obligationCodes.map((code) => [code, legacy._activeServices?.[code] !== false]),
    ),
  };
}
...
export async function up(connection: PoolConnection) {
  ...
  if (!(await columnExists(connection, 'obligation_municipalities', 'service_config'))) {
    await connection.query(
      `ALTER TABLE obligation_municipalities
  ...
  if (await columnExists(connection, 'obligation_municipalities', 'responsible_config')) {
    const [municipalities]: any = await connection.query(
      'SELECT id, name, responsible_config, service_config FROM obligation_municipalities',
    );
    for (const municipality of municipalities) {
      const legacy = parseConfig(municipality.responsible_config);
      if (Object.keys(legacy).length > 0) {
        await connection.query(
          `INSERT IGNORE INTO obligation_legacy_responsibility_archive
            (municipality_id, municipality_name, responsible_config) VALUES (?, ?, ?)`,
          [municipality.id, municipality.name, JSON.stringify(legacy)],
        );
      }
      ...
      await connection.query(
        'UPDATE obligation_municipalities SET service_config = ? WHERE id = ?',
        [JSON.stringify(serviceConfig), municipality.id],
      );
    }
  ...
  await connection.query(
    `UPDATE obligation_municipalities
     SET service_config = ? WHERE service_config IS NULL`,
    [JSON.stringify({ activeServices: Object.fromEntries(obligationCodes.map((code) => [code, true])) })],
  );
  ...
  for (const [key, description] of permissionDescriptions) {
    await connection.query(
      'UPDATE permissions_catalog SET descricao = ? WHERE permission_key = ?',
      [description, key],
    );
  }
  await connection.query(
    `UPDATE permissions_catalog SET nome = 'Desativar municípios'
...
export async function down(connection: PoolConnection) {
  ...
  const [archiveRows]: any = await connection.query(
    'SELECT municipality_id, responsible_config FROM obligation_legacy_responsibility_archive',
  );
  for (const row of archiveRows) {
    await connection.query(
      'UPDATE obligation_municipalities SET responsible_config = ? WHERE id = ?',
      [JSON.stringify(parseConfig(row.responsible_config)), row.municipality_id],
    );
  }
}
