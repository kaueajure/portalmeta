...
export async function up(connection: PoolConnection) {
  const [companies]: any = await connection.query(
    'SELECT id, nome FROM empresas WHERE id = ? LIMIT 1',
    [SOURCE_COMPANY_ID]
  );
  ...
  for (const table of scopedTables) {
    const [rows]: any = await connection.query(
      `SELECT COUNT(*) AS total FROM \`${table}\` WHERE empresa_id = ?`,
      [SOURCE_COMPANY_ID]
    );
    await connection.query(
      `INSERT INTO single_company_migration_audit
         (source_company_id, entity_name, row_count)
       VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE row_count = VALUES(row_count), captured_at = CURRENT_TIMESTAMP`,
      [SOURCE_COMPANY_ID, table, Number(rows[0]?.total || 0)]
    );
  }
}
