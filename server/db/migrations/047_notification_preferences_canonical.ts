...
async function columnExists(connection: PoolConnection, column: string): Promise<boolean> {
  ...
  for (const [legacy, canonical] of legacyMappings) {
    if (await columnExists(connection, legacy)) {
      await connection.query(
        `UPDATE notification_preferences SET \`${canonical}\` = \`${legacy}\``,
      );
      await connection.query(
        `ALTER TABLE notification_preferences DROP COLUMN \`${legacy}\``,
      );
    }
  }
}
