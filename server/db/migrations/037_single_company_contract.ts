...
async function dropForeignKeys(connection: PoolConnection) {
  const [rows]: any = await connection.query(
    `SELECT DISTINCT TABLE_NAME, CONSTRAINT_NAME
     ...
     WHERE TABLE_SCHEMA = DATABASE()
       ...
       AND (COLUMN_NAME IN (?) OR REFERENCED_TABLE_NAME = 'empresas')`,
    [SCOPED_COLUMNS],
  );
...
async function dropScopedIndexes(connection: PoolConnection) {
  const [rows]: any = await connection.query(
...
async function dropScopedColumns(connection: PoolConnection) {
  const [rows]: any = await connection.query(
...
async function renameTable(connection: PoolConnection, from: string, to: string) {
  const [rows]: any = await connection.query(
  ...
  if (targetRows.length > 0) throw new Error(`Contrato single-company abortado: as tabelas ${from} e ${to} coexistem.`);
  await connection.query(`RENAME TABLE \`${from}\` TO \`${to}\``);
}

async function dropIndexIfExists(connection: PoolConnection, table: string, index: string) {
  const [rows]: any = await connection.query(
    `SELECT INDEX_NAME FROM information_schema.STATISTICS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND INDEX_NAME = ? LIMIT 1`,
    [table, index],
  );
  if (rows.length > 0) await connection.query(`ALTER TABLE \`${table}\` DROP INDEX \`${index}\``);
}

async function addUniqueIndex(connection: PoolConnection, table: string, index: string, columns: string) {
  const [rows]: any = await connection.query(
    `SELECT INDEX_NAME FROM information_schema.STATISTICS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND INDEX_NAME = ? LIMIT 1`,
    [table, index],
...
export async function up(connection: PoolConnection) {
  const [companies]: any = await connection.query('SELECT id, nome FROM empresas ORDER BY id');
  ...
  const [scopedTables]: any = await connection.query(
    `SELECT TABLE_NAME
     FROM information_schema.COLUMNS
  ...
  const [portalColumn]: any = await connection.query(
