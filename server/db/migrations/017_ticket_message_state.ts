...
export async function up(connection: PoolConnection) {
  ...
  if (!(await columnExists(connection, 'tickets', 'aguardando_resposta_atendente'))) {
    await connection.query(
      'ALTER TABLE tickets ADD COLUMN aguardando_resposta_atendente TINYINT(1) NOT NULL DEFAULT 0'
    );
  }
  if (!(await columnExists(connection, 'tickets', 'ultima_mensagem_publica_em'))) {
    await connection.query(
      'ALTER TABLE tickets ADD COLUMN ultima_mensagem_publica_em DATETIME NULL'
    );
  }
  if (!(await columnExists(connection, 'tickets', 'ultima_mensagem_publica_origem'))) {
    await connection.query(
      "ALTER TABLE tickets ADD COLUMN ultima_mensagem_publica_origem VARCHAR(20) NULL"
    );
  }
  ...
  if (!(await indexExists(connection, 'tickets', 'idx_tickets_empresa_aguardando'))) {
    await connection.query(
      'ALTER TABLE tickets ADD INDEX idx_tickets_empresa_aguardando (empresa_id, aguardando_resposta_atendente)'
    );
  }
