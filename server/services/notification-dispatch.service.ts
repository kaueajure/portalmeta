...
function safePreview(value: unknown, max = 140): string {
...
async function activeCandidates(category: NotificationCategory, onlyUserId?: number): Promise<CandidateUser[]> {
  const column = preferenceColumn(category);
  const [rows]: any = await pool.query(
    `SELECT u.id, u.nome, u.perfil, u.administrador, u.desenvolvedor, u.access_profile_id,
            np.${column} AS enabled
     FROM usuarios u
     LEFT JOIN notification_preferences np ON np.usuario_id = u.id
     WHERE u.ativo = 1 AND u.perfil <> 'cliente' ${onlyUserId ? 'AND u.id = ?' : ''}`,
    onlyUserId ? [onlyUserId] : [],
  );
  return rows.filter((row: CandidateUser) => row.enabled === null || Number(row.enabled) === 1);
}

async function canUseNotifications(user: CandidateUser): Promise<boolean> {
...
export const notificationDispatchService = {
  ...
  async ticket(input: {
    ...
    for (const user of candidates) {
      ...
      if (canAccessTicketByScope(ticket, user, scope)) recipientIds.push(Number(user.id));
    }
  ...
  async whatsapp(input: {
    ...
    const category: NotificationCategory = assigned ? 'whatsapp_assigned' : 'whatsapp_general';
